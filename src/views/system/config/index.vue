  ...
  <Container>
    <el-form ref="ruleFormRef" style="max-width: 700px" :model="ruleForm" :rules="rules" label-width="180px" :size="formSize" status-icon>
      ...
      <el-form-item label="终端默认登陆方式" prop="defaultLoginType">
        <el-select v-model="ruleForm.defaultLoginType" style="width: 240px" placeholder="请选择终端默认登陆方式">
      ...
      <el-form-item label="终端工作步骤停留时间" prop="type">
        <div class="flex">
          <div class="flex items-start gap-2 mr-10">
            <el-input-number v-model="ruleForm.bestTimes" :controls="false" style="width: 50px" />
            <span>最短时间/秒</span>
          </div>
          <div class="flex items-start gap-2">
            <el-input v-model="ruleForm.maximumDuration" :controls="false" style="width: 50px" />
            <span>最长时间/秒</span>
          </div>
        </div>
      </el-form-item>
      <el-form-item>
        <Button type="primary" @click="submitForm(ruleFormRef)">
          保 存
        </Button>
      </el-form-item>
    </el-form>
