...
<template>
  <div class="text-center">
    <Form @register="register" class="p-4">
      <template #requestIp>
        <div class="w-[580px] text-left">{{ requestIp }}</div>
      </template>
      <template #methods>
        <div class="w-[580px] text-left">{{ requestIp }}</div>
      </template>
      ...
      <template #status>
        <div class="w-[580px] text-left">{{ searchOptions.status.find((item) => item.id === status)?.dictLabel }}</div>
      </template>
      <template #createTime>
        <div class="w-[580px] text-left">{{ createTime }}</div>
      </template>
    </Form>
