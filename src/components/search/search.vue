...
<template>
  <el-form @keydown.enter="callbackFun('query')">
    <div ref="el" class="box" :class="[border ? 'border border-solid border-[#cfe4fa] p-[15px]' : '']">
      <slot name="main">
      ...
      <Button :icon="Search" @click="callbackFun('query')">
        查询
      </Button>
      <Button v-if="reset" :icon="Refresh" @click="callbackFun('reset')">
        重置
      </Button>
